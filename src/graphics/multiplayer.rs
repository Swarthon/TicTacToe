...
pub fn update(window : &conrod::backend::piston::Window, ids : &graphics::Ids, ui : &mut conrod::Ui, player : char, terrain : [[char;3];3], node : &ai::Node) -> graphics::State{
		...
		for y in 0..3 {
				...
				.set(ids.case[x*3+y], &mut ui)
			{
				...
				match basic::test_win(&terrain){
					'X'	=> return graphics::State::End('X'),
					'O'	=> return graphics::State::End('O'),
					'0'	=> return graphics::State::End(' '),
					_	=> continue,
				}
			}
		}
	...
	graphics::State::Solo(player, terrain)
}
